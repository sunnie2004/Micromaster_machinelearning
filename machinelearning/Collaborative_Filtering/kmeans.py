...
def estep(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    for i in range(n):
        tiled_vector = np.tile(X[i, :], (K, 1))   #copy K times by Y axis, and 1 time by X axis
        sse = ((tiled_vector - mixture.mu)**2).sum(axis=1) #add by the row,[1,1]+[2,3] =[2,5],but axis=0 is [3,4]
        j = np.argmin(sse)
        post[i, j] = 1
