...
def compute_accuracy(predictions, y):
    ...
    return np.mean(np.equal(predictions.numpy(), y.numpy()))   #if equal then true, otherwise, false, get the total equal pairs/total pairs
...
def run_epoch(data, model, optimizer):
    ...
    for batch in tqdm(data):
        ...
        predictions_first_label = torch.argmax(out1, dim=1)   #get the maximum
        ...
        # Compute both losses
        loss1 = F.cross_entropy(out1, y[0])  #loss function
