...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        output_layer_vectorize = np.vectorize(output_layer_activation_derivative)

        hidden_layer_vectorize = np.vectorize(rectified_linear_unit_derivative)

        output_layer_error = (-1)*(y-activated_output)* (output_layer_vectorize(output)) # TODO

        hidden_layer_error =  (-1)*(y-activated_output) * (output_layer_vectorize(output)) * \
                                self.hidden_to_output_weights * \
                             (hidden_layer_vectorize(hidden_layer_weighted_input))
                             # TODO (3 by 1 matrix)

        bias_gradients = np.array(np.transpose(self.hidden_to_output_weights)) * \
                         np.array(hidden_layer_vectorize(hidden_layer_weighted_input)) * \
                         np.array(activated_output - y) * np.array(output_layer_vectorize(output))    # TODO

        hidden_to_output_weight_gradients = -(y-activated_output) * (output_layer_vectorize(output)) * \
                                            np.transpose(hidden_layer_activation)# TODO
######################
   ##    attention: multiple and dot and mulmat difference between array and matrix 03-24-2020
        #########################

        input_to_hidden_weight_gradients = -np.array(y-activated_output) * \
                                           np.array(output_layer_vectorize(output)) * \
                                           np.array(hidden_layer_vectorize(hidden_layer_weighted_input)) * \
                                           np.array(np.transpose(self.hidden_to_output_weights)) * np.array(np.transpose(input_values))
