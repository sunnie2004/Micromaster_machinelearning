...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    center_X = X - feature_means    #n*d
    # eigenvectors = principal_components(center_X)   #d*d
    return np.matmul(center_X,pcs[:,0:n_components])
...
def reconstruct_PC(x_pca, pcs, n_components, X):
    ...
    """
    center_X,feature_means = center_data(X)
